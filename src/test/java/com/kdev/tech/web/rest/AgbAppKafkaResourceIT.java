...
import com.kdev.tech.config.KafkaSseConsumer;
import com.kdev.tech.config.KafkaSseProducer;
...
class AgbAppKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/agb-app-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/agb-app-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/agb-app-kafka/unregister"));
                return;
            }
        }
